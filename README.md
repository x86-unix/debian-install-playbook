...
site-hypervisor.yml        # KVM ハイパーバイザー（virt-manager + noVNC）
...
extra/
  ...
  extra-hypervisor.yml   # KVM ハイパーバイザー（virt-manager + noVNC HTTP）
...
`site-hypervisor.yml` で KVM + virt-manager 環境を構築し、ブラウザから virt-manager デスクトップにアクセスできるようにする。

### 前提条件

- BIOS/UEFI で CPU 仮想化支援（VT-x/AMD-V）を有効にすること
- `site-extra.yml` 実行済み（ただし `extra-remote-desktop.yml` とは排他）

### 実行

```bash
ansible-playbook -i inventory.ini site-hypervisor.yml
```
...
```mermaid
graph LR
    A[labwc + virt-manager] --> B[wayvnc<br>127.0.0.1:5900]
    B --> C[websockify<br>127.0.0.1:6080]
    C --> D[Apache<br>0.0.0.0:80]
    D --> E[ブラウザ<br>複数ユーザー同時アクセス可]
```

### アクセス

ブラウザから `http://<host>/` でアクセス（認証なし、LAN 内利用想定）。複数ユーザーが同時に同じ virt-manager デスクトップを操作できる。

### ポートバインド確認

```bash
ss -ltpn | grep -E ':(80|6080|5900)'
```

正常な状態：

```
0.0.0.0:80     ← Apache（HTTP、認証なし）
127.0.0.1:6080 ← noVNC websockify（ローカルのみ）
127.0.0.1:5900 ← wayvnc（ローカルのみ）
```

### 注意

- `extra-remote-desktop.yml`（HTTPS + OTP）とは排他。hypervisor マシンでは本 playbook のみ使用する
- Apache はポート 80 で認証なし公開のため、信頼できる LAN 内でのみ使用すること
